...
| 원인 | 해결 |
|------|------|
| 텍스트 수정 후 linesegarray 미삭제 | 수정된 `<hp:p>`에서 `<linesegarray>` 자식 요소를 반드시 삭제. clone_form.py 사용 시 자동 처리됨 |
| lxml 직접 편집 시 누락 | `_remove_linesegarray_from_p(p)` 호출 필수. 또는 `_remove_all_linesegarray(root)`로 전체 제거 |

> **원리**: linesegarray는 한컴오피스가 저장한 "줄 배치 캐시"다. 텍스트를 변경하면 캐시가 무효화되어 글자가 겹쳐 보인다. 삭제하면 한컴오피스가 파일을 열 때 자동으로 줄 배치를 재계산한다.
